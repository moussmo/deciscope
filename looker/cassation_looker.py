...
def get_decisions(endpoint, header):
    url = endpoint + 'export'
...
def reduce_decisions(decisions):
    return decisions
...
def look_cassation_decisions():
    ...
    decisions = get_decisions(endpoint, header)
    filtered_decisions = filter_decisions(decisions, start_datetime, end_datetime)
    reduced_decisions = reduce_decisions(filtered_decisions)
